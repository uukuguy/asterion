...
test("runs a real SDK prompt through the inherited duplex bridge without exposing private values in command results", async () => {
  ...
  try {
    ...
    assert.ok(
      firstModel.payload.context.messages.some((item) => item.role === "user"),
    );
    send(
      command(3, firstModel.request_id, "model.response", {
    ...
    assert.deepEqual(tool.payload, {
      tool_call_id: "call-1",
      code: "SENTINEL_TOOL_INPUT",
    });
    send(
      command(4, tool.request_id, "tool.response", {
        result: {
          content: [{ type: "text", text: "SENTINEL_TOOL_OUTPUT" }],
          details: {},
          isError: false,
        },
      }),
    ...
    send(
      command(5, secondModel.request_id, "model.response", {
        message: assistantDoneMessage(),
      }),
    );
    const completed = await next();
    assert.equal(completed.kind, "command.result");
    assert.deepEqual(completed.payload.result.assistant, {
      completed: true,
      stop_reason: "stop",
    });
    assert.doesNotMatch(
      JSON.stringify(completed),
      /SENTINEL_(PROMPT|TOOL_INPUT|TOOL_OUTPUT|MODEL_COMPLETION)/,
    );
...
test("keeps bridge inspection free of transport and path sentinels", async () => {
  const { P1DevelopmentBridge } =
    await import("../dist/src/p1-development-bridge.js");
    ...
    content: [
      {
        ...
        id: "call-1",
        name: "ipython",
        arguments: { code: "SENTINEL_TOOL_INPUT" },
      },
    ],
    usage: usage(),
    stopReason: "toolUse",
...
function assistantDoneMessage() {
  return {
    ...
    provider: "asterion-development",
    model: "p1-test",
    content: [{ type: "text", text: "SENTINEL_MODEL_COMPLETION" }],
    usage: usage(),
    stopReason: "stop",
    timestamp: Date.now(),
  };
