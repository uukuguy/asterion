...
test("keeps one P6 SDK session for three bounded staged prompts", async () => {
  const { PrimeP6DevelopmentSession } = await import("../dist/src/index.js");
  const { createAssistantMessageEventStream } = await import(pathToFileURL(join(
    primeSourceRoot,
    "node_modules/@earendil-works/pi-ai/dist/utils/event-stream.js",
  )).href);
  const workspace = await mkdtemp(join(tmpdir(), "asterion-p6-sdk-"));
  let models = 0, tools = 0;
  const contexts = [];
  const session = await PrimeP6DevelopmentSession.open({
  ...
  try {
    ...
    assert.ok(contexts.some((context) => context.includes(
      "complete the required P6 staged artifact creation",
    )));
