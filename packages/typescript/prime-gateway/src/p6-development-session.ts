...
export interface PrimeP6DevelopmentSessionOptions {
...
export interface PrimeP6DevelopmentUsage {
  readonly input_tokens: number;
  readonly output_tokens: number;
  readonly total_tokens: number;
}

export interface PrimeP6DevelopmentResult {
  readonly lifecycle: "completed" | "cancelled";
  readonly usage: PrimeP6DevelopmentUsage;
...
export interface PrimeP6DevelopmentSdkSession {
  readonly session: PrimeSdkSession;
  unregister(): void;
  readonly control: { state: "open" | "cancelled" | "closed" };
  observations(): PrimeP6DevelopmentResult["observations"];
}

/** Internal construction seam for P6's bounded repair session. */
export async function openPrimeP6DevelopmentSdkSession(
  options: PrimeP6DevelopmentSessionOptions,
...
): Promise<PrimeP6DevelopmentSdkSession> {
  ...
  const provider = `asterion-p6-development-${identity}`;
  const modelId = `p6-development-${identity}`;
  ...
  registry.registerProvider(provider, {
    api: `asterion-p6-development-${identity}`, baseUrl: "http://127.0.0.1:0", apiKey: "in-memory-development-provider",
    streamSimple: (model: unknown, context: unknown, streamOptions: unknown) => {
      assertCallbackAllowed(control);
      if (++modelCalls > limits.model) throw new Error("Prime P6 development model callback limit exceeded");
  ...
  if (!model) throw new Error("Prime P6 development model registration failed");
  const agentDir = join(options.workspace, ".asterion-prime-p6-development");
  ...
  const customIpython = {
    name: "ipython", description: "P6 bounded-stage IPython bridge. The prompt supplies the fixed paths and known bytes. This one call must complete the required P6 staged artifact creation; do not spend it on inspection, printing, validation, or subprocess execution.", label: "ipython",
    ...
    execute: async (toolCallId: string, input: { code: string }, signal: AbortSignal) => {
      assertCallbackAllowed(control);
      if (++toolCalls > limits.tool) throw new Error("Prime P6 development ipython callback limit exceeded");
...
export class PrimeP6DevelopmentSession {
  #state: "open" | "cancelled" | "closed" = "open";
  readonly #session: PrimeSdkSession;
  readonly #unregister: () => void;
  readonly #control: { state: "open" | "cancelled" | "closed" };
  readonly #observations: () => PrimeP6DevelopmentResult["observations"];
  ...
  static async open(
    options: PrimeP6DevelopmentSessionOptions,
  ): Promise<PrimeP6DevelopmentSession> {
    const sdk = await openPrimeP6DevelopmentSdkSession(options, {
      model: MAX_MODEL_CALLS,
      tool: MAX_TOOL_CALLS,
    });
    return new PrimeP6DevelopmentSession(
      sdk.session,
      sdk.unregister,
      sdk.control,
      sdk.observations,
  ...
  async prompt(prompt: string): Promise<PrimeP6DevelopmentResult> {
  ...
  private assertDispatchable(): void {
    if (this.#state === "cancelled")
      throw new Error("Prime P6 development session is cancelled");
    if (this.#state === "closed")
      throw new Error("Prime P6 development session is closed");
  }

  private result(
    lifecycle: "completed" | "cancelled",
  ): PrimeP6DevelopmentResult {
  ...
  [inspect.custom](): string {
    return "PrimeP6DevelopmentSession { lifecycle: safe }";
  }
}
...
function isAssistant(
  ...
  stopReason?: PrimeP6DevelopmentResult["assistant"]["stop_reason"];
...
function safeStopReason(
  value: unknown,
): PrimeP6DevelopmentResult["assistant"]["stop_reason"] {
  return value === "stop" ||
    value === "length" ||
    value === "toolUse" ||
    value === "error" ||
