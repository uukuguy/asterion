...
import { PrimeP7DevelopmentSession } from "./p7-development-session.js";
import type { PrimeSdkAssistantMessageEventStream } from "./p7-development-session.js";

export const P7_DEVELOPMENT_GATEWAY_PROTOCOL =
  "asterion.prime-p7-development-gateway/v1";
export const P7_DEVELOPMENT_MAX_FRAME_BYTES = 1024 * 1024;
...
type F = Readonly<
  I & {
    protocol: typeof P7_DEVELOPMENT_GATEWAY_PROTOCOL;
    sequence: number;
    request_id: string;
    kind: K;
...
export class P7DevelopmentBridgeError extends Error {
  constructor() {
    super("P7 development bridge failed");
  }
}
const SAFE = Object.freeze({
  role: "assistant",
  api: "anthropic-messages",
  provider: "asterion-p7-development",
  model: "p7-development",
  content: [],
  usage: {
    input: 0,
    output: 0,
...
class Decoder {
  ...
  push(c: Uint8Array): F[] {
    ...
    while (this.#b.length >= 4) {
      ...
      if (n > P7_DEVELOPMENT_MAX_FRAME_BYTES)
  ...
  ["nodejs.util.inspect.custom"]() {
    return "P7DevelopmentFrameDecoder {}";
  }
}
/** Development-only inherited-FD bridge. The child must be launched with an empty environment. */
export class P7DevelopmentBridge {
  #d = new Decoder();
  #in = 0;
  #out = 0;
  #i: I | undefined;
  #s: PrimeP7DevelopmentSession | undefined;
  #p:
    | "new"
    | "opening"
    | "open"
  ...
  ["nodejs.util.inspect.custom"]() {
    return `P7DevelopmentBridge { phase: ${this.#p} }`;
  }
  async run(): Promise<void> {
    try {
      ...
      throw new P7DevelopmentBridgeError();
    } finally {
      await this.dispose();
    }
  }
  #validate(f: F) {
    if (
      !rec(f) ||
      !only(f, KEYS) ||
      f.protocol !== P7_DEVELOPMENT_GATEWAY_PROTOCOL ||
  ...
  async command(f: F) {
    if (f.kind === "open") {
      ...
      this.#s = await PrimeP7DevelopmentSession.open({
    ...
    if (f.kind === "prompt") {
      ...
      if (++this.#promptCount > 3)
        throw Error("invalid P7 development prompt flow");
    ...
    if (f.kind === "feedback") {
      if (!only(f.payload, ["feedback"]) || this.#promptCount !== 1 || this.#feedback !== undefined)
        throw Error("invalid P7 development feedback");
  ...
  emit(
    ...
    const b = Buffer.from(
      canon({
        protocol: P7_DEVELOPMENT_GATEWAY_PROTOCOL,
        ...this.#i,
        sequence: ++this.#out,
        request_id,
        kind,
    ...
    if (b.length > P7_DEVELOPMENT_MAX_FRAME_BYTES)
      throw Error("frame exceeds limit");
...
export function inheritedP7DevelopmentSocket(fd: number) {
