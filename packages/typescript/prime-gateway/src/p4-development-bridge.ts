...
export class P4DevelopmentBridge {
  ...
  async #command(f: Frame): Promise<void> {
    ...
    if (f.kind === "close") { if (!exact(f.payload, [])) throw Error("close payload"); this.#settle(Error("closed")); const detached = await this.#client?.request({ type: "detach", activeSessionId: this.#activeSessionId }); if (!detached?.success) throw Error("close detach"); this.#emit("command.result", f.request_id, { result: this.#finalWitness() }); await this.#client.request({ type: "shutdown" }); this.#phase = "closed"; this.#socket.end(); return; }
    throw Error("command");
  }
  async #open(f: Frame): Promise<void> {
    ...
    const sessionId = readSessionId(attached?.data?.snapshot); if (!sessionId) throw Error("native session identity"); this.#nativeSessionId = sessionId;
  }
  ...
  async #checkpointCandidate(): Promise<Record<string, unknown>> {
    ...
    if (!header?.success || !tree?.success || !resources?.success || header?.data?.header?.id !== this.#nativeSessionId) throw Error("checkpoint identity");
  ...
  async #recover(f: Frame): Promise<void> {
    ...
    const candidate = this.#candidate as any; const attached = await requireP4DetachedBeforeAttach(this.#client.request({ type: "detach", activeSessionId: this.#activeSessionId }), () => this.#attach(candidate.cursor)), data = attached?.data, replay = data?.replay, last = data?.lastEventCursor, snapshot = data?.snapshot?.lastEventCursor;
    if (!attached?.success || data.activeSessionId !== candidate.active_session_id || readSessionId(data.snapshot) !== candidate.session_id || replay?.status !== "complete" || !sameCursor(replay.fromCursor, candidate.cursor) || !sameCursor(replay.toCursor, candidate.cursor) || !sameCursor(last, candidate.cursor) || !sameCursor(snapshot, candidate.cursor)) throw Error("reattach mismatch");
    this.#phase = "compact"; this.#emit("command.result", f.request_id, { result: { active_session_id: candidate.active_session_id, session_id: candidate.session_id, from_cursor: replay.fromCursor, to_cursor: replay.toCursor, snapshot_cursor: snapshot, replay_status: replay.status } });
  }
  async #compact(f: Frame): Promise<void> {
    if (!exact(f.payload, [])) throw Error("compact payload"); const manager = this.#runtime?.session?.sessionManager, before = new Set((manager?.getEntries?.() ?? []).filter((e: any) => e.type === "compaction").map((e: any) => e.id)), oldLeaf = manager?.getLeafId?.();
    ...
    const entries = (manager?.getEntries?.() ?? []).filter((e: any) => e.type === "compaction" && !before.has(e.id));
    const first = native?.firstKeptEntryId, tokens = native?.tokensBefore;
    const entry = entries[0];
    if (!result?.success || this.#compactionStarts !== 1 || this.#compactionEnds !== 1 || entries.length !== 1 || typeof first !== "string" || !Number.isSafeInteger(tokens) || tokens < 0 || this.#modelCount !== 3 || this.#toolCount !== 1 || entry?.firstKeptEntryId !== first || entry?.tokensBefore !== tokens || typeof entry?.summary !== "string" || !entry.summary || entry.parentId !== oldLeaf || manager?.getLeafId?.() !== entry.id) throw Error("compaction witness");
    this.#phase = "prompt2"; this.#emit("command.result", f.request_id, { result: { compact_called: true, succeeded: true, start_count: 1, end_count: 1, new_entry_count: 1, active_path_sha256: digest(manager?.getFlatTree?.()), first_kept_entry_id_sha256: digest(first), tokens_before: tokens } });
  }
  ...
  async #startNative(root: string, workspace: string): Promise<void> {
    ...
    const provider = "asterion-p4-development", modelId = "p4-development", apiKey = "in-memory-development-provider"; auth.setRuntimeApiKey(provider, apiKey); registry.registerProvider(provider, { api: provider, baseUrl: "http://127.0.0.1:0", apiKey, streamSimple: (model: unknown, context: unknown, options: unknown) => this.#modelStream(root, model, context, options), models: [{ id: modelId, name: modelId, reasoning: false, input: ["text"], contextWindow: 16384, maxTokens: 1024, cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 } }] }); const model = registry.find(provider, modelId); if (!model) throw Error("model");
    ...
    const socket = join(workspace, "p4.sock"); if (process.platform !== "win32" && Buffer.byteLength(socket) >= 104) throw Error("socket path"); this.#native = new daemonModule.AgentDaemon(socket, { defaultSessionConfig: { cwd: workspace, agentDir, sessionDir: agentDir, telemetryDisabled: true }, createRuntime: runtime }); await this.#native.start(); this.#client = new clientModule.DaemonClient(socket); this.#client.onMessage((message: any) => { const cursor = message?.meta?.cursor; if (typeof cursor?.generation === "string" && Number.isSafeInteger(cursor.sequence)) this.#cursor = { generation: cursor.generation, sequence: cursor.sequence }; }); await this.#client.connect(); const hello = await this.#client.waitForHello(); if (hello?.protocol?.version !== 7 || hello?.schemaId !== "protocol-7-schema-14-816309b1cd50" || hello?.schemaRevision !== 14 || hello?.appVersion !== "0.7.1" || typeof hello?.runtime?.buildId !== "string" || !hello.runtime.buildId || typeof hello?.clientId !== "string" || !hello.clientId || !["attach_snapshot", "event_sequence"].every((x) => hello?.serverCapabilities?.includes(x)) || "supervisorGeneration" in hello) throw Error("hello");
  }
  ...
  async #dispose(): Promise<void> { this.#settle(Error("closed")); try { this.#client?.close(); } catch {} }
}
...
/** Keeps the destructive recovery boundary ordered for both the daemon and tests. */
export async function requireP4DetachedBeforeAttach<T>(detached: PromiseLike<unknown>, attach: () => Promise<T>): Promise<T> { const result = await detached; if (!record(result) || result.success !== true) throw Error("detach failed"); return attach(); }
...
function digest(value: unknown): string { return `sha256:${createHash("sha256").update(canonical(json(value ?? null))).digest("hex")}`; }
...
function readSessionId(snapshot: any): string | undefined { return opaque(snapshot?.summary?.sessionId) ? snapshot.summary.sessionId : undefined; }
