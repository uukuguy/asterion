...
export const P1_DEVELOPMENT_GATEWAY_PROTOCOL =
  "asterion.prime-p1-development-gateway/v1";
...
type F = Readonly<
  I & {
    protocol: typeof P1_DEVELOPMENT_GATEWAY_PROTOCOL;
    sequence: number;
    request_id: string;
    kind: K;
...
export class P1DevelopmentBridgeError extends Error {
  constructor() {
    super("P1 development bridge failed");
  }
}
const SAFE = Object.freeze({
  role: "assistant",
  api: "anthropic-messages",
  provider: "asterion-p1-development",
  model: "p1-development",
  content: [],
  usage: {
    input: 0,
    output: 0,
...
class Decoder {
  ...
  push(c: Uint8Array): F[] {
    ...
    while (this.#b.length >= 4) {
      ...
      if (n > P1_DEVELOPMENT_MAX_FRAME_BYTES)
  ...
  ["nodejs.util.inspect.custom"]() {
    return "P1DevelopmentFrameDecoder {}";
  }
}
...
export class P1DevelopmentBridge {
  ...
  #out = 0;
  #i: I | undefined;
  #s: PrimeP1DevelopmentSession | undefined;
  #p:
    | "new"
    | "opening"
    | "open"
  ...
  ["nodejs.util.inspect.custom"]() {
    return `P1DevelopmentBridge { phase: ${this.#p} }`;
  }
  async run(): Promise<void> {
    try {
      ...
      throw new P1DevelopmentBridgeError();
    } finally {
      await this.dispose();
    }
  }
  #validate(f: F) {
    if (
      !rec(f) ||
      !only(f, KEYS) ||
      f.protocol !== P1_DEVELOPMENT_GATEWAY_PROTOCOL ||
  ...
  async command(f: F) {
    if (f.kind === "open") {
      ...
      this.#s = await PrimeP1DevelopmentSession.open({
  ...
  emit(
    ...
    const b = Buffer.from(
      canon({
        protocol: P1_DEVELOPMENT_GATEWAY_PROTOCOL,
        ...this.#i,
        sequence: ++this.#out,
        request_id,
        kind,
    ...
    if (b.length > P1_DEVELOPMENT_MAX_FRAME_BYTES)
      throw Error("frame exceeds limit");
...
export function inheritedP1DevelopmentSocket(fd: number) {
