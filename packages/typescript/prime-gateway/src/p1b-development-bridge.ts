...
import { PrimeP1BDevelopmentSession } from "./p1b-development-session.js";
import type { PrimeSdkAssistantMessageEventStream } from "./p1-development-session.js";

export const P1B_DEVELOPMENT_GATEWAY_PROTOCOL =
  "asterion.prime-p1-b-development-gateway/v1";
export const P1B_DEVELOPMENT_MAX_FRAME_BYTES = 1024 * 1024;
...
type K =
  "open" | "prompt" | "compact" | "cancel" | "close" | "model.response" | "tool.response";
type I = Readonly<{
  run_id: string;
  session_id: string;
...
type F = Readonly<
  I & {
    protocol: typeof P1B_DEVELOPMENT_GATEWAY_PROTOCOL;
    sequence: number;
    request_id: string;
    kind: K;
...
export class P1BDevelopmentBridgeError extends Error {
  constructor() {
    super("P1B development bridge failed");
  }
}
const SAFE = Object.freeze({
  role: "assistant",
  api: "anthropic-messages",
  provider: "asterion-p1-b-development",
  model: "p1-b-development",
  content: [],
  usage: {
    input: 0,
    output: 0,
...
class Decoder {
  ...
  push(c: Uint8Array): F[] {
    ...
    while (this.#b.length >= 4) {
      ...
      if (n > P1B_DEVELOPMENT_MAX_FRAME_BYTES)
  ...
  ["nodejs.util.inspect.custom"]() {
    return "P1BDevelopmentFrameDecoder {}";
  }
}
/** Development-only inherited-FD bridge. The child must be launched with an empty environment. */
export class P1BDevelopmentBridge {
  #d = new Decoder();
  #in = 0;
  #out = 0;
  #i: I | undefined;
  #s: PrimeP1BDevelopmentSession | undefined;
  #p:
    | "new"
    | "opening"
    | "open"
  ...
  ["nodejs.util.inspect.custom"]() {
    return `P1BDevelopmentBridge { phase: ${this.#p} }`;
  }
  async run(): Promise<void> {
    try {
      ...
      throw new P1BDevelopmentBridgeError();
    } finally {
      await this.dispose();
    }
  }
  #validate(f: F) {
    if (
      !rec(f) || !only(f, KEYS) || f.protocol !== P1B_DEVELOPMENT_GATEWAY_PROTOCOL ||
      f.runtime_id !== "prime.agent" || !Number.isSafeInteger(f.generation) || f.generation < 1 ||
      !opaque(f.run_id) || !opaque(f.session_id) || !opaque(f.request_id) ||
      !Number.isSafeInteger(f.sequence) || f.sequence !== ++this.#in || !rec(f.payload) || !kind(f.kind)
    ) throw Error("invalid bridge frame");
    const i: I = { run_id: f.run_id, session_id: f.session_id, runtime_id: f.runtime_id, generation: f.generation };
    if (!this.#i) { if (f.kind !== "open") throw Error("bridge must open first"); this.#i = i; }
    else if (!same(this.#i, i)) throw Error("bridge identity mismatch");
    if (f.kind.endsWith("response")) {
      if (!(this.#p === "prompt1" || this.#p === "compact" || this.#p === "prompt2")) throw Error("unexpected bridge response");
      return;
    }
    if (this.#ids.has(f.request_id)) throw Error("duplicate command request");
    const allowed: Record<string, readonly string[]> = {
      new: ["open"], open: ["prompt", "cancel"], prompt1: ["cancel"],
      compact: ["compact", "cancel"], prompt2: ["prompt", "cancel"],
      close_ready: ["close"], cancelled: ["close"],
    };
    if (!allowed[this.#p]?.includes(f.kind)) throw Error("invalid bridge phase");
  }
  dispatch(f: F) {
    this.#validate(f);
  ...
  async command(f: F) {
    if (f.kind === "open") {
      ...
      this.#s = await PrimeP1BDevelopmentSession.open({
    ...
    if (f.kind === "compact") {
      if (!only(f.payload, [])) throw Error("invalid compact payload");
  ...
  session(): PrimeP1BDevelopmentSession {
  ...
  emit(
    ...
    const b = Buffer.from(
      canon({
        protocol: P1B_DEVELOPMENT_GATEWAY_PROTOCOL,
        ...this.#i,
        sequence: ++this.#out,
        request_id,
        kind,
    ...
    if (b.length > P1B_DEVELOPMENT_MAX_FRAME_BYTES)
      throw Error("frame exceeds limit");
...
export function inheritedP1BDevelopmentSocket(fd: number) {
