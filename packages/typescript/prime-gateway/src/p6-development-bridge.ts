...
import { PrimeP6DevelopmentSession } from "./p6-development-session.js";
import type { PrimeSdkAssistantMessageEventStream } from "./p6-development-session.js";

export const P6_DEVELOPMENT_GATEWAY_PROTOCOL =
  "asterion.prime-p6-development-gateway/v1";
export const P6_DEVELOPMENT_MAX_FRAME_BYTES = 1024 * 1024;
...
type F = Readonly<
  I & {
    protocol: typeof P6_DEVELOPMENT_GATEWAY_PROTOCOL;
    sequence: number;
    request_id: string;
    kind: K;
...
export class P6DevelopmentBridgeError extends Error {
  constructor() {
...
const SAFE = Object.freeze({
  ...
  provider: "asterion-p6-development",
  model: "p6-development",
  content: [],
  usage: {
    input: 0,
    output: 0,
...
class Decoder {
  ...
  push(c: Uint8Array): F[] {
    ...
    while (this.#b.length >= 4) {
      ...
      if (n > P6_DEVELOPMENT_MAX_FRAME_BYTES)
  ...
  ["nodejs.util.inspect.custom"]() {
    return "P6DevelopmentFrameDecoder {}";
  }
}
/** Development-only inherited-FD bridge. The child must be launched with an empty environment. */
export class P6DevelopmentBridge {
  #d = new Decoder();
  #in = 0;
  #out = 0;
  #i: I | undefined;
  #s: PrimeP6DevelopmentSession | undefined;
  #p:
    | "new"
    | "opening"
    | "open"
  ...
  ["nodejs.util.inspect.custom"]() {
    return `P6DevelopmentBridge { phase: ${this.#p} }`;
  }
  async run(): Promise<void> {
    try {
      ...
      throw new P6DevelopmentBridgeError();
    } finally {
      await this.dispose();
    }
  }
  #validate(f: F) {
    if (
      !rec(f) ||
      !only(f, KEYS) ||
      f.protocol !== P6_DEVELOPMENT_GATEWAY_PROTOCOL ||
  ...
  async command(f: F) {
    if (f.kind === "open") {
      ...
      this.#s = await PrimeP6DevelopmentSession.open({
    ...
    if (f.kind === "prompt") {
      ...
      if (++this.#promptCount > 3)
        throw Error("invalid P6 development prompt flow");
    ...
    if (f.kind === "feedback") {
      if (!only(f.payload, ["feedback"]) || this.#promptCount !== 1 || this.#feedback !== undefined)
        throw Error("invalid P6 development feedback");
  ...
  emit(
    ...
    const b = Buffer.from(
      canon({
        protocol: P6_DEVELOPMENT_GATEWAY_PROTOCOL,
        ...this.#i,
        sequence: ++this.#out,
        request_id,
        kind,
    ...
    if (b.length > P6_DEVELOPMENT_MAX_FRAME_BYTES)
      throw Error("frame exceeds limit");
...
export function inheritedP6DevelopmentSocket(fd: number) {
