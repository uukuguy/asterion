...
import { PrimeP3DevelopmentSession } from "./p3-development-session.js";
import type { PrimeP3DevelopmentRole, PrimeSdkAssistantMessageEventStream } from "./p3-development-session.js";

export const P3_DEVELOPMENT_GATEWAY_PROTOCOL =
  "asterion.prime-p3-development-gateway/v1";
export const P3_DEVELOPMENT_MAX_FRAME_BYTES = 1024 * 1024;
...
type K =
  "open" | "prompt" | "cancel" | "close" | "model.response" | "tool.response" |
  "rlm.spawn" | "rlm.wait" | "rlm.follow_up" | "rlm.list" | "rlm.delete";
type I = Readonly<{
  run_id: string;
  session_id: string;
...
type F = Readonly<
  I & {
    protocol: typeof P3_DEVELOPMENT_GATEWAY_PROTOCOL;
    sequence: number;
    request_id: string;
    kind: K;
...
export class P3DevelopmentBridgeError extends Error {
  constructor() {
    super("P3 development bridge failed");
  }
}
...
class Decoder {
  ...
  push(c: Uint8Array): F[] {
    ...
    while (this.#b.length >= 4) {
      ...
      if (n > P3_DEVELOPMENT_MAX_FRAME_BYTES)
  ...
  ["nodejs.util.inspect.custom"]() {
    return "P3DevelopmentFrameDecoder {}";
  }
}
/** Development-only inherited-FD bridge. The child must be launched with an empty environment. */
export class P3DevelopmentBridge {
  #d = new Decoder();
  #in = 0;
  #out = 0;
  #i: I | undefined;
  #s: PrimeP3DevelopmentSession | undefined;
  #p:
    | "new"
    | "opening"
    | "open"
  ...
  ["nodejs.util.inspect.custom"]() {
    return `P3DevelopmentBridge { phase: ${this.#p} }`;
  }
  async run(): Promise<void> {
    try {
      ...
      throw new P3DevelopmentBridgeError();
    } finally {
      await this.dispose();
    }
  }
  #validate(f: F) {
    if (
      !rec(f) ||
      !only(f, KEYS) ||
      f.protocol !== P3_DEVELOPMENT_GATEWAY_PROTOCOL ||
    ...
    if (this.#p === "prompt" && !(["cancel", "rlm.spawn", "rlm.wait", "rlm.follow_up", "rlm.list", "rlm.delete"] as string[]).includes(f.kind))
      throw Error("command interleaving is forbidden");
    if (
      (this.#p === "new" && f.kind !== "open") ||
      (this.#p === "open" &&
        !(["prompt", "cancel", "close"] as string[]).includes(f.kind)) ||
  ...
  async command(f: F) {
    if (f.kind === "open") {
      ...
      this.#s = await PrimeP3DevelopmentSession.open({
        primeSourceRoot: root,
        workspace,
        model: (role, m, c, o) => this.#model(role, m, c, o, make),
        ipython: (role, id, input) => this.#tool(role, id, input.code),
      });
      this.#p = "open";
    ...
    if (f.kind === "rlm.spawn") {
      if (!only(f.payload, ["role", "prompt"]) ||
        (f.payload.role !== "implementation" && f.payload.role !== "review") || typeof f.payload.prompt !== "string")
        throw Error("invalid private payload");
      const result = await this.session().spawn(f.payload.role, f.payload.prompt);
      this.emit("command.result", f.request_id, { result });
      return;
    }
    if (f.kind === "rlm.wait") {
      const childId = text(f.payload, "child_id");
      await this.session().wait(childId);
      this.emit("command.result", f.request_id, { result: {} });
      return;
    }
    if (f.kind === "rlm.follow_up") {
      if (!only(f.payload, ["child_id", "prompt"]) || typeof f.payload.child_id !== "string" || typeof f.payload.prompt !== "string")
        throw Error("invalid private payload");
      await this.session().followUp(f.payload.child_id, f.payload.prompt);
      this.emit("command.result", f.request_id, { result: {} });
      return;
    }
    if (f.kind === "rlm.list") {
      if (!only(f.payload, [])) throw Error("invalid private payload");
      this.emit("command.result", f.request_id, { result: await this.session().list() });
      return;
    }
    if (f.kind === "rlm.delete") {
      const childId = text(f.payload, "child_id");
      await this.session().delete(childId);
      this.emit("command.result", f.request_id, { result: {} });
      return;
    }
    if (f.kind === "cancel") {
  ...
  #tool(role: PrimeP3DevelopmentRole, tool_call_id: string, code: string): Promise<unknown> {
    const id = this.callback("tool");
    return new Promise((resolve, reject) => {
      this.#t.set(id, { resolve, reject });
      try {
        this.emit("tool.request", id, { role, tool_call_id, code });
      } catch (e) {
        this.#t.delete(id);
  ...
  emit(
    ...
    const b = Buffer.from(
      canon({
        protocol: P3_DEVELOPMENT_GATEWAY_PROTOCOL,
        ...this.#i,
        sequence: ++this.#out,
        request_id,
        kind,
    ...
    if (b.length > P3_DEVELOPMENT_MAX_FRAME_BYTES)
      throw Error("frame exceeds limit");
...
export function inheritedP3DevelopmentSocket(fd: number) {
