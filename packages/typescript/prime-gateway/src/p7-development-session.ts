...
export interface PrimeP7DevelopmentSessionOptions {
...
export interface PrimeP7DevelopmentUsage {
  readonly input_tokens: number;
  readonly output_tokens: number;
  readonly total_tokens: number;
}

export interface PrimeP7DevelopmentResult {
  readonly lifecycle: "completed" | "cancelled";
  readonly usage: PrimeP7DevelopmentUsage;
...
export interface PrimeP7DevelopmentSdkSession {
  readonly session: PrimeSdkSession;
  unregister(): void;
  readonly control: { state: "open" | "cancelled" | "closed" };
  observations(): PrimeP7DevelopmentResult["observations"];
}

/** Internal construction seam for P7's bounded repair session. */
export async function openPrimeP7DevelopmentSdkSession(
  options: PrimeP7DevelopmentSessionOptions,
...
): Promise<PrimeP7DevelopmentSdkSession> {
  ...
  const provider = `asterion-p7-development-${identity}`;
  const modelId = `p7-development-${identity}`;
  ...
  registry.registerProvider(provider, {
    api: `asterion-p7-development-${identity}`, baseUrl: "http://127.0.0.1:0", apiKey: "in-memory-development-provider",
    streamSimple: (model: unknown, context: unknown, streamOptions: unknown) => {
      assertCallbackAllowed(control);
      if (++modelCalls > limits.model) throw new Error("Prime P7 development model callback limit exceeded");
  ...
  if (!model) throw new Error("Prime P7 development model registration failed");
  const agentDir = join(options.workspace, ".asterion-prime-p7-development");
  ...
  const customIpython = {
    name: "ipython", description: "P7 bounded-stage IPython bridge. The prompt supplies the fixed paths and known bytes. This one call must complete the required P7 staged artifact creation; do not spend it on inspection, printing, validation, or subprocess execution.", label: "ipython",
    ...
    execute: async (toolCallId: string, input: { code: string }, signal: AbortSignal) => {
      assertCallbackAllowed(control);
      if (++toolCalls > limits.tool) throw new Error("Prime P7 development ipython callback limit exceeded");
...
export class PrimeP7DevelopmentSession {
  #state: "open" | "cancelled" | "closed" = "open";
  readonly #session: PrimeSdkSession;
  readonly #unregister: () => void;
  readonly #control: { state: "open" | "cancelled" | "closed" };
  readonly #observations: () => PrimeP7DevelopmentResult["observations"];
  ...
  static async open(
    options: PrimeP7DevelopmentSessionOptions,
  ): Promise<PrimeP7DevelopmentSession> {
    const sdk = await openPrimeP7DevelopmentSdkSession(options, {
      model: MAX_MODEL_CALLS,
      tool: MAX_TOOL_CALLS,
    });
    return new PrimeP7DevelopmentSession(
      sdk.session,
      sdk.unregister,
      sdk.control,
      sdk.observations,
  ...
  async prompt(prompt: string): Promise<PrimeP7DevelopmentResult> {
  ...
  private assertDispatchable(): void {
    if (this.#state === "cancelled")
      throw new Error("Prime P7 development session is cancelled");
    if (this.#state === "closed")
      throw new Error("Prime P7 development session is closed");
  }

  private result(
    lifecycle: "completed" | "cancelled",
  ): PrimeP7DevelopmentResult {
  ...
  [inspect.custom](): string {
    return "PrimeP7DevelopmentSession { lifecycle: safe }";
  }
}
...
function isAssistant(
  ...
  stopReason?: PrimeP7DevelopmentResult["assistant"]["stop_reason"];
...
function safeStopReason(
  value: unknown,
): PrimeP7DevelopmentResult["assistant"]["stop_reason"] {
  return value === "stop" ||
    value === "length" ||
    value === "toolUse" ||
    value === "error" ||
