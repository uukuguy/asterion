...
import { PrimeP2DevelopmentSession } from "./p2-development-session.js";
import type { PrimeSdkAssistantMessageEventStream } from "./p2-development-session.js";

export const P2_DEVELOPMENT_GATEWAY_PROTOCOL =
  "asterion.prime-p2-development-gateway/v1";
export const P2_DEVELOPMENT_MAX_FRAME_BYTES = 1024 * 1024;
...
type F = Readonly<
  I & {
    protocol: typeof P2_DEVELOPMENT_GATEWAY_PROTOCOL;
    sequence: number;
    request_id: string;
    kind: K;
...
export class P2DevelopmentBridgeError extends Error {
  constructor() {
...
const SAFE = Object.freeze({
  ...
  provider: "asterion-p2-development",
  model: "p2-development",
  content: [],
  usage: {
    input: 0,
    output: 0,
...
class Decoder {
  ...
  push(c: Uint8Array): F[] {
    ...
    while (this.#b.length >= 4) {
      ...
      if (n > P2_DEVELOPMENT_MAX_FRAME_BYTES)
  ...
  ["nodejs.util.inspect.custom"]() {
    return "P2DevelopmentFrameDecoder {}";
  }
}
/** Development-only inherited-FD bridge. The child must be launched with an empty environment. */
export class P2DevelopmentBridge {
  #d = new Decoder();
  #in = 0;
  #out = 0;
  #i: I | undefined;
  #s: PrimeP2DevelopmentSession | undefined;
  #p:
    | "new"
    | "opening"
    | "open"
  ...
  ["nodejs.util.inspect.custom"]() {
    return `P2DevelopmentBridge { phase: ${this.#p} }`;
  }
  async run(): Promise<void> {
    try {
      ...
      throw new P2DevelopmentBridgeError();
    } finally {
      await this.dispose();
    }
  }
  #validate(f: F) {
    if (
      !rec(f) ||
      !only(f, KEYS) ||
      f.protocol !== P2_DEVELOPMENT_GATEWAY_PROTOCOL ||
  ...
  async command(f: F) {
    if (f.kind === "open") {
      ...
      this.#s = await PrimeP2DevelopmentSession.open({
  ...
  emit(
    ...
    const b = Buffer.from(
      canon({
        protocol: P2_DEVELOPMENT_GATEWAY_PROTOCOL,
        ...this.#i,
        sequence: ++this.#out,
        request_id,
        kind,
    ...
    if (b.length > P2_DEVELOPMENT_MAX_FRAME_BYTES)
      throw Error("frame exceeds limit");
...
export function inheritedP2DevelopmentSocket(fd: number) {
