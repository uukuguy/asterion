...
async def _reap_process(
    process: DockerCliAttachProcess, *, deadline: float | None = None
) -> None:
...
class P1BDockerCliTransport(DockerCliEngineTransport):
        ...
        *,
        ...
        try:
            ...
            create_control = _LifecycleCallControl(
                monotonic() + _CREATE_DEADLINE_SECONDS,
                control.signal,
            )
            ...
            self._specifications[daemon] = _P1BSpec(
                daemon,
                image_digest,
                "prime.ipython-coding-p1b-development",
                run_id,
                "sha256:" + "0" * 64,
                _DIGEST,
            )  # type: ignore[attr-defined]
            ...
            task = asyncio.create_task(self._compensate_provisional(requested))
            while not task.done():
                try:
    ...
    async def _compensate_provisional(self, name: str) -> None:
        ...
        async def remove_then_inspect(control: _LifecycleCallControl) -> bool:
            ...
            inspected = await self._call_raw(
                self._prefix + ("container", "inspect", "--format", "{{.Id}}", name),
                control,
            )  # type: ignore[attr-defined]
            if (
    ...
    async def inspect(
        ...
        exact = {
            ...
            "VolumesFrom": None,
            "Tmpfs": {
                "/workspace": "rw,nodev,noexec,nosuid,size=67108864,uid=65534,gid=65534,mode=0700"
            },
            "PidsLimit": 256,
            "Memory": 536870912,
        ...
        if (
            set(values) != set(exact) | {"Env", "PortBindings", "SecurityOpt"}
            or any(values[key] != value for key, value in exact.items())
            or not self._valid_environment(values["Env"])
            or (
                values["PortBindings"] is not None
                and (
                    type(values["PortBindings"]) is not dict
                    or values["PortBindings"] != {}
                )
            )
            or values["SecurityOpt"]
            != ["no-new-privileges:true", "seccomp=" + self._seccomp_profile]  # type: ignore[attr-defined]
            or result.stderr
        ):
            raise RestrictedWorkerError("restricted worker value is invalid")
    ...
    async def snapshot(
        ...
        try:
            continuity = await self._call(
                self._prefix
                + (
                    "container",
                    "cp",
                    container_id + ":/workspace/p1b-state/continuity.txt",
                    "-",
                ),
                control,
            )  # type: ignore[attr-defined]
            if self._archive_file(continuity.stdout, "continuity.txt") != _FIXTURE:
                raise ValueError
            solution = await self._call(
                self._prefix
                + ("container", "cp", container_id + ":/workspace/solution.py", "-"),
                control,
            )  # type: ignore[attr-defined]
            ...
            await self._call(
                self._prefix + ("container", "unpause", container_id), cleanup
            )  # type: ignore[attr-defined]
    ...
    async def initial_snapshot(
        ...
        try:
            ...
            await self._call(
                self._prefix + ("container", "unpause", container_id),
                _LifecycleCallControl(monotonic() + 30, None),
            )  # type: ignore[attr-defined]
    ...
    def _archive_file(raw: bytes, name: str) -> bytes:
        with tarfile.open(fileobj=io.BytesIO(raw), mode="r:") as tar:
            ...
            if (
                len(members) != 1
                or members[0].name != name
                or not members[0].isreg()
                or members[0].size > _FRAME_CAP
            ):
                raise ValueError
            ...
            if len(data) != members[0].size or len(data) > _FRAME_CAP:
                raise ValueError
            return data
...
class P1BDockerCompletion:
    ...
    def __post_init__(self) -> None:
        if (
            self.workload_digest != _DIGEST
            or self.kernel_generation != 1
            or self.cell_count != 2
            or self.probe_count != 12
        ):
            raise RestrictedWorkerError("restricted worker value is invalid")
...
class _P1BChannel:
    ...
    def __init__(
        self, process: DockerCliAttachProcess, *, run_id: str, session_id: str
    ) -> None:
        self._process, self._identity = (
            process,
    ...
    async def self_check(
        ...
        if self._state != "self-check":
            raise RestrictedWorkerError("restricted worker value is invalid")
        self._state = "cell-1"
        return DockerCliEngineTransport._parse_self_check_line(
            await self._read(control)
        )

    async def execute_cell(
        self, cell: str, *, control: _LifecycleCallControl
    ) -> dict[str, object]:
        ...
        event = self._event(
            await self._read(control),
            sequence,
            "baseline.recorded" if sequence == 1 else "continuity.verified",
        )
        expected = (
            {"baseline_recorded", "cell_count", "kernel_generation", "probe_count"}
            if sequence == 1
            else {"cell_count", "kernel_generation", "preserved", "probe_count"}
        )
        if (
            set(event) != expected
            or event["cell_count"] != sequence
            or event["kernel_generation"] != 1
            or event["probe_count"] != sequence * 6
        ):
            raise RestrictedWorkerError("restricted worker value is invalid")
        if sequence == 1 and event["baseline_recorded"] is not True:
            raise RestrictedWorkerError("restricted worker value is invalid")
        if sequence == 2 and (
            type(event["preserved"]) is not dict
            or set(event["preserved"])
            != {
                "cwd",
                "file_bytes",
                "function_behavior",
            ...
            or not all(value is True for value in event["preserved"].values())
        ):
            raise RestrictedWorkerError("restricted worker value is invalid")
    ...
    async def finish(self, *, control: _LifecycleCallControl) -> P1BDockerCompletion:
        ...
        if (
            set(event)
            != {"cell_count", "completed", "kernel_generation", "probe_count"}
            or event["completed"] is not True
        ):
            raise RestrictedWorkerError("restricted worker value is invalid")
        self._state = "completed"
        return P1BDockerCompletion(
            _DIGEST,
            event["kernel_generation"],
            event["cell_count"],
            event["probe_count"],
        )
    ...
    async def _read(self, control: _LifecycleCallControl) -> bytes:
        ...
        while b"\n" not in raw and len(raw) <= 1024:
            async with asyncio.timeout_at(control.deadline):
                raw += await self._process.stdout.read(1025)
    ...
    async def _write(
        self, value: dict[str, object], control: _LifecycleCallControl
    ) -> None:
    ...
    def _event(self, raw: bytes, sequence: int, kind: str) -> dict[str, object]:
        ...
        return {
            key: value[key]
            for key in value
            if key not in {"protocol", "identity", "sequence", "kind"}
        }
...
class P1BDockerPersistentWorkerService:
    ...
    def __init__(
        ...
        *,
        ...
        if not (
            type(image_digest) is str
            and image_digest.startswith("sha256:")
            and len(image_digest) == 71
            and all(c in "0123456789abcdef" for c in image_digest[7:])
            ...
            raise RestrictedWorkerError("restricted worker value is invalid")
        self._image, self._transport, self._run, self._session = (
            image_digest,
            transport,
            run_id,
            session_id,
        ...
        self._channel: _P1BChannel | None = None
        self._state = "new"

    def __repr__(self) -> str:
        return "P1BDockerPersistentWorkerService(redacted)"
    ...
    async def acquire(self) -> None:
        ...
        try:
            container = await self._transport.create(
            ...
            await self._transport.inspect(container, control=control)
            await self._transport.start(container, control=control)
            channel = await self._transport.channel(
                container, run_id=self._run, session_id=self._session, control=control
            )
            self._channel = channel
            check = await channel.self_check(control=control)
            if not (
                check.nonloopback_network_absent
                and check.root_read_only
                and check.workspace_only_writable
                and check.credentials_absent
                and check.effective_capabilities == 0
                and check.no_new_privileges == 1
                and check.seccomp_mode == 2
                and check.effective_user_id == 65534
            ):
                raise ValueError
            self._state = "cell-1"
        except asyncio.CancelledError:
    ...
    async def execute_cell(self, cell: str) -> dict[str, object]:
        if self._channel is None or self._state not in {"cell-1", "cell-2"}:
            raise RestrictedWorkerError("restricted worker value is invalid")
        result = await self._channel.execute_cell(
            cell, control=_LifecycleCallControl(monotonic() + 30, None)
        )
    ...
    async def initial_snapshot(self) -> bytes:
        if self._container is None or self._state != "cell-1":
            raise RestrictedWorkerError("restricted worker value is invalid")
        value = await self._transport.initial_snapshot(
            self._container, control=_LifecycleCallControl(monotonic() + 30, None)
        )
    ...
    async def finish(self) -> P1BDockerCompletion:
        if self._channel is None or self._state != "finish":
            raise RestrictedWorkerError("restricted worker value is invalid")
        result = await self._channel.finish(
    ...
    async def snapshot(self) -> bytes:
        if self._container is None or self._state != "snapshot":
            raise RestrictedWorkerError("restricted worker value is invalid")
        value = await self._transport.snapshot(
            self._container, control=_LifecycleCallControl(monotonic() + 30, None)
        )
        if type(value) is not bytes or not value or len(value) > _FRAME_CAP:
            raise RestrictedWorkerError("restricted worker value is invalid")
        ...
        async def destroy() -> bool:
            ...
            if channel is not None:
                try:
                    ...
                    close_error = True
            await self._transport.force_remove(
                container, control=_LifecycleCallControl(monotonic() + 30, None)
            )
            await self._transport.assert_absent(
                container, control=_LifecycleCallControl(monotonic() + 30, None)
            )
            return close_error
