"""Installed local-root host for the fixed P5 continuity verification."""
...
_APPLICATION_ID = "prime.bounded-autonomy"
...
def create_prime_p5_cli_factory(*, repo_root: Path) -> HostServiceFactoryBinding:
...
def create_host_service_factory() -> HostServiceFactoryBinding:
    return create_prime_p5_cli_factory(repo_root=Path.cwd())
...
def _sealed_seccomp(path: Path) -> int:
    ...
    try:
        descriptor = os.memfd_create(
            "asterion-p5-development-seccomp", os.MFD_CLOEXEC | os.MFD_ALLOW_SEALING
