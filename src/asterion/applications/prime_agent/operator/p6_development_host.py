...
def _prompt(stage: int, run_id: str, session_id: str, candidate_sha256: str | None = None) -> str:
    prompts = {
        1: f"P6 stage 1: run_id {run_id}; session_id {session_id}; observation-only. MUST load and call the existing clamp from /workspace/baseline.py; MUST NOT define, replace, repair, or write clamp or baseline.py. inputs MUST equal exactly [[-4,-2,3]]: one row only and no extra cases. Calling the baseline clamp(value, lower, upper) must produce actual outputs exactly [-4] as a one-integer list, and passed MUST be false. The independent oracle expected outputs are [-2]; do not substitute oracle output for observed baseline output. Write /workspace/task-a.json using fixture clamp-task-a/v1 stage task-a and fields fixture, stage, run_id, session_id, source_sha256 prefixed sha256:, inputs, outputs, passed. Encode canonical compact sorted-key UTF-8 JSON. Use one completion-only IPython call that performs the write; no printing, inspection-only calls, imports, subprocess, scope, authority, or rollback.",
