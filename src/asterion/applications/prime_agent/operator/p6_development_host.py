...
def _prompt(stage: int, run_id: str, session_id: str, candidate_sha256: str | None = None) -> str:
    prompts = {
        1: f"P6 stage 1: run_id {run_id}; session_id {session_id}; compute from /workspace/baseline.py and write /workspace/task-a.json. For each input [value,lower,upper], call clamp(value, lower, upper) as three arguments; outputs is the integer list, and passed compares those outputs with expected clamp results. Use fixture clamp-task-a/v1 stage task-a and fields fixture, stage, run_id, session_id, source_sha256 prefixed sha256:, inputs, outputs, passed. Encode canonical compact sorted-key UTF-8 JSON. Use one completion-only IPython call that performs the write; no printing, inspection-only calls, subprocess, scope, authority, or rollback.",
    ...
    if stage == 3 and _DIGEST.fullmatch(candidate_sha256 or ""):
        return f"P6 stage 3: run_id {run_id}; session_id {session_id}; selected candidate digest is {candidate_sha256}; read /workspace/candidate.py and write /workspace/task-b.json. For each input [value,lower,upper], call clamp(value, lower, upper) as three arguments; outputs is the integer list, and passed compares those outputs with expected clamp results. Use fixture clamp-task-b/v1 stage task-b and fields fixture, stage, run_id, session_id, source_sha256 prefixed sha256:, inputs, outputs, passed. Encode canonical compact sorted-key UTF-8 JSON. Use one completion-only IPython call that performs the write; no printing, inspection, subprocess, scope, authority, or rollback."
    if stage in prompts:
