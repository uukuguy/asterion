...
class PrimeP2DevelopmentSdkProviderError(ValueError):
...
class PrimeP2DevelopmentSdkProvider:
    ...
    def __init__(self, config: _PrivatePrimeModelConfig) -> None:
        if type(config) is not _PrivatePrimeModelConfig:
            raise PrimeP2DevelopmentSdkProviderError()
        self._calls = 0
    ...
    def __repr__(self) -> str:
        return "PrimeP2DevelopmentSdkProvider(redacted)"
    ...
    async def __call__(self, body: bytes) -> bytes:
        ...
        try:
            ...
            raise PrimeP2DevelopmentSdkProviderError() from None
...
def create_prime_p2_development_sdk_provider(
    operator_config: Mapping[str, object],
) -> PrimeP2DevelopmentSdkProvider:
    try:
        return PrimeP2DevelopmentSdkProvider(_private_config_from_values(operator_config))
    except BaseException:
        raise PrimeP2DevelopmentSdkProviderError() from None
...
def _deepseek_payload(request: object, model_id: str, turn: int, max_output: int) -> dict[str, object]:
    ...
    return {
        ...
        # The verified P1-compatible endpoint accepts automatic tool selection;
        # the host still rejects anything except one IPython call on turn one.
...
__all__ = (
    "PrimeP2DevelopmentSdkProvider",
    "PrimeP2DevelopmentSdkProviderError",
    "create_prime_p2_development_sdk_provider",
)
