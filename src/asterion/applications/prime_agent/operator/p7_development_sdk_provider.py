"""Private six-stage DeepSeek adapter for the structured P7 SDK bridge."""
...
_INPUT_CAP = 128 * 1024
_MAX_REQUESTS = 6
_TOTAL_INPUT_TOKENS = 49_152
_TOTAL_OUTPUT_TOKENS = 3_456
_TOTAL_COST_MICROUNITS = 30_000
_OUTPUT_LIMITS = (1024, 128, 1024, 128, 1024, 128)
...
P7_PROVIDER_CALLBACK_LIMIT = _MAX_REQUESTS
P7_PROVIDER_INPUT_LIMIT = _TOTAL_INPUT_TOKENS
P7_PROVIDER_OUTPUT_LIMIT = _TOTAL_OUTPUT_TOKENS
P7_PROVIDER_COST_LIMIT = _TOTAL_COST_MICROUNITS
P7_PROVIDER_DEADLINE_SECONDS = int(_DEADLINE_SECONDS)
P7_PROVIDER_OUTPUT_LIMITS = _OUTPUT_LIMITS


class PrimeP7DevelopmentSdkProviderError(ValueError):
    """Body-free P7 development SDK provider failure."""

    def __init__(self, *_: object) -> None:
        super().__init__("prime P7 development SDK provider is unavailable")


class PrimeP7DevelopmentSdkProvider:
    """Translate exactly six private P7 SDK callbacks."""
    ...
    def __init__(self, config: _PrivatePrimeModelConfig) -> None:
        if type(config) is not _PrivatePrimeModelConfig:
            raise PrimeP7DevelopmentSdkProviderError()
        self._calls = 0
    ...
    def __repr__(self) -> str:
        return "PrimeP7DevelopmentSdkProvider(redacted)"
...
def create_prime_p7_development_sdk_provider(
    operator_config: Mapping[str, object],
) -> PrimeP7DevelopmentSdkProvider:
    try:
        return PrimeP7DevelopmentSdkProvider(
...
def _validate_request(
    ...
    if turn % 2:
        ...
        if (
            ...
            or any(_canonical_json(answer) not in _canonical_json(messages) for _, answer in issued)
        ):
            raise ValueError
...
def _deepseek_payload(
    ...
    *,
    ...
    if type(turn) is not int or turn not in range(6):
        raise ValueError
...
__all__ = (
    "PrimeP7DevelopmentSdkProvider",
    "PrimeP7DevelopmentSdkProviderError",
    "create_prime_p7_development_sdk_provider",
    "P7_PROVIDER_OUTPUT_LIMITS",
)
