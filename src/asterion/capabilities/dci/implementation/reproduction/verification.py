...
from asterion.capabilities.dci.implementation.evaluation.evaluation import evaluate_run_directory
...
from asterion.capabilities.dci.implementation.runtime.run import DciRunRequest, DciRunResult, run_pi_research
...
def paper_product_contract() -> dict[str, object]:
    ...
    from asterion.capabilities.dci.implementation.research.experiment_profiles import experiment_profile_ids
...
def paper_benchmark_resource_digests() -> tuple[tuple[str, str], ...]:
    ...
    validate_paper_ablation_matrix()
    root = resources.files("asterion.capabilities.dci.resources")
    ...
    def acceptance(self, acceptance_root: Path | None) -> VerificationResult:
        ...
        try:
            if self.acceptance_checks is None:
                raise ValueError("installed acceptance checker is unavailable")
