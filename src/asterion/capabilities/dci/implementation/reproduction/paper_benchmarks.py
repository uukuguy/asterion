...
def _load_resource(name: str, schema: str, collection: str) -> dict[str, Any]:
    try:
        raw = resources.files("asterion.capabilities.dci.resources").joinpath(name).read_text(
            encoding="utf-8"
        )
...
def _fixture_ids() -> Mapping[str, tuple[str, str]]:
    ...
    if (
        ...
        raise RuntimeError("DCI paper benchmark contract is invalid")
    resource_root = resources.files("asterion.capabilities.dci.resources")
...
def require_af320_executable_scope(
    ...
    from asterion.capabilities.dci.implementation.research.experiment_profiles import consume_full_execution_authorization
