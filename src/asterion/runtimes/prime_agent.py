...
@dataclass(frozen=True)
class PrimePresetExecutionProfile:
    input_preset: str
    scope: str
    promotion: str
    artifact_id: str
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise PrimeAgentRuntimeError("Prime preset runtime profile is invalid")
...
class PrimePresetRuntimeClient(AgentRuntimeClient):
    """Project one host-owned preset execution into runtime protocol frames."""
    ...
    def __init__(
        ...
        *,
        ...
        self._service = service
        if type(profile) is not PrimePresetExecutionProfile:
            raise PrimeAgentRuntimeError("Prime preset runtime profile is invalid")
        self._profile = profile
    ...
    async def run(
        ...
        *,
        ...
        try:
            ...
            raise PrimeAgentRuntimeError("Prime runtime request is invalid") from error
        if (
            request.input_text != self._profile.input_preset
            or request.requested_capabilities != (PRIME_IPYTHON_CAPABILITY,)
            or request.deadline_ms is not None
        ):
            raise PrimeAgentRuntimeError("Prime runtime tool is not declared")
        ...
        yield RunEvent(
            ...
            payload={
                "artifact": {
                    ...
                    "kind": self._profile.kind,
                    "media_type": self._profile.media_type,
                    "sha256": result.receipt_sha256.removeprefix("sha256:"),
                }
            },
        )
        yield RunEvent(
...
def _failed_events(run_id: str) -> tuple[RunEvent, RunEvent]:
    return (
        RunEvent(
            run_id=run_id,
            sequence=1,
