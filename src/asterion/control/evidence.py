...
class ControlEvidenceProjector:
    ...
    def complete_open_spans(self, *, timestamp_ns: int) -> None:
        try:
            self._complete_open_spans(timestamp_ns=timestamp_ns)
        except Exception:
            self._record_gap()
    ...
    def _project_execution(
        ...
        *,
        ...
        attributes: dict[str, SafeAttributeValue] = {
            ...
            "control_event_type": f"action.{status}",
    ...
    def _complete_open_spans(self, *, timestamp_ns: int) -> None:
        if self._disabled or self._session_span_id is None:
            return
        assert self._trace_id is not None
        records = [
        ...
        if self._system_span_id is not None:
            records.append(
                TraceEvent.complete(
                    self._trace_id,
                    self._system_span_id,
                    self._recorder.next_sequence + 1,
                    kind="system",
                    timestamp_ns=timestamp_ns,
                )
            )
        self._record_many(tuple(records))
        self._session_span_id = None
        self._system_span_id = None
...
def _add_event_payload_attributes(
    ...
    if event.type == "action.proposed":
        ...
        if isinstance(kind, str):
            attributes["scope_sha256"] = _digest(kind)
        if isinstance(target, Mapping):
            attributes["request_shape_sha256"] = _mapping_digest(dict(target))
