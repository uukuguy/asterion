...
_PROBE_GOAL = (
    ...
    "call help or shell tools, write files, or perform unrelated work. Its first cell "
    "must execute exactly this native-RLM call:\n"
    "child = await rlm(\"Reply pong to your parent using agent_message.send with "
    "receiver_role='parent', then stop.\", name=\"native-rlm-child\")\n"
    "After admission, poll `await rlm.list_subagents()` until this child has a nonempty "
    "active_session_id, sleeping 0.25 seconds between polls. Then execute exactly one "
    ...
    "message. When the child's ordinary reply starts the next parent turn, execute "
    "`child = (await rlm.list_subagents())[0]` followed by "
    "`await rlm.delete_subagent(child)`, then finish."
)
