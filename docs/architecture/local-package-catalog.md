...
validated `asterion.capability/v1` manifests. Roots are explicit configuration
owned by the local caller; they are not accepted from an agent request, model
response, capability manifest, or provider payload.
...
- Every JSON document must be an object conforming to the canonical capability
  validator.
...
directories, or mutate catalog roots. It also does not execute capabilities,
