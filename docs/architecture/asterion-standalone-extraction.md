...
- DCI benchmark task bindings：由 capability package 的 Python implementation
  提供；逐任务 shell 入口已在后续迁移中删除。
...
| `asterion/scripts` | `scripts` | 仅迁移通用 setup 工具 | benchmark task 使用 package-owned bindings |
...
2. 迁移 14 个 package-owned benchmark task binding，并确认它们由通用 benchmark subsystem 解析，不回退到旧 `src/dci`。
...
- DCI 应用适配器、通用 benchmark plan/run/resume 契约和 package-owned bindings
  通过；historical mixed-repository 的 launcher pair 和 538/538 delegated selector
  不是当前 standalone 验收。
