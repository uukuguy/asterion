...
| Native `Verified-loop` | Missing | `make test.native-verified-loop.provider-free` | Nine provider-free rows are evidenced; the parameter-free small preset is implemented, but its operator-owned host is not configured. |
