# Research Tree — climb cycle observability

> Generated by `tools/climb/regen-tree.py` (deterministic — 7 runs logged)
> Do NOT edit — re-generated on every push / LB landed / cycle complete.

**SOTA**: —

## In-flight / session state (dynamic — resume reads this, then verifies liveness)

- **phase**: Plan 4 Task 8 implementation
- **last_cycle**: 7
- **next_hypothesis**: H-008

**In-flight job** (resume MUST verify liveness):
- kind: subagent-driven-development
- task: Plan 4 Task 8
- external_execution: False

**Next action on resume**: Dispatch Plan 4 Task 8 implementer with TDD brief.

## Paradigm calibration matrix

| paradigm | n | mean_gap | std | last_3 |
|---|---|---|---|---|

## Push ladder (chronological)

| run_id | paradigm | parent | local | online | gap | verdict |
|---|---|---|---|---|---|---|
| plan4-climb-h001 | plan4-task-1 | — | 1.00 | — | — | 🟢 confirmed |
| plan4-climb-h002 | plan4-task-2 | — | 2.00 | — | — | 🟢 confirmed |
| plan4-climb-h003 | plan4-task-3 | — | 3.00 | — | — | 🟢 confirmed |
| plan4-climb-h004 | plan4-task-4 | — | 4.00 | — | — | 🟢 confirmed |
| plan4-climb-h005 | plan4-task-5 | — | 5.00 | — | — | 🟢 confirmed |
| plan4-climb-h006 | plan4-task-6 | — | 6.00 | — | — | 🟢 confirmed |
| plan4-climb-h007 | plan4-task-7 | — | 7.00 | — | — | 🟢 confirmed |

## Mermaid push DAG

```mermaid
graph LR
    classDef sota fill:#4ade80,stroke:#15803d,color:#000
    classDef confirmed fill:#86efac,stroke:#16a34a,color:#000
    classDef falsified fill:#fca5a5,stroke:#b91c1c,color:#000
    classDef pending fill:#fde68a,stroke:#a16207,color:#000
    plan4_climb_h001["plan4-climb-h001<br/>—"]
    class plan4_climb_h001 confirmed
    plan4_climb_h002["plan4-climb-h002<br/>—"]
    class plan4_climb_h002 confirmed
    plan4_climb_h003["plan4-climb-h003<br/>—"]
    class plan4_climb_h003 confirmed
    plan4_climb_h004["plan4-climb-h004<br/>—"]
    class plan4_climb_h004 confirmed
    plan4_climb_h005["plan4-climb-h005<br/>—"]
    class plan4_climb_h005 confirmed
    plan4_climb_h006["plan4-climb-h006<br/>—"]
    class plan4_climb_h006 confirmed
    plan4_climb_h007["plan4-climb-h007<br/>—"]
    class plan4_climb_h007 confirmed
```

## Hypothesis pool

### Active (1) — ranked by priority
- **H-008** (rank 0.65, cost 3.0h, in-flight): Close migration and repository boundaries
  - lift: +1 completed task

### Confirmed (7)
- H-001: Establish the complete portable DCI payload
- H-002: Replace shell launchers with exact DCI task bindings
- H-003: Move all DCI implementation and resources under the package
- H-004: Reduce the DCI CLI to an application adapter
- H-005: Remove global DCI benchmark orchestration and launchers
- H-006: Prove DCI first as an installed third-party distribution
- H-007: Materialize the identical payload as a built-in source form

