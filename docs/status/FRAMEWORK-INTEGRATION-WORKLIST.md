...
| W1b | Lockable source candidate flow | One source-neutral host step turns metadata candidates into digest-bearing candidates before exact lock resolution; builtin, local, and distribution sources share the same lifecycle without provider import during discovery | Active | W1a |
| W1c | Framework/product boundary | `runtime/defaults.py` has no Prime application routing; provider-owned integration supplies exact bindings. Agent runtime and control-plane responsibilities are documented as orthogonal contracts before Native routing changes | Planned | W1b |
| W1d | Core-only boundary | Core import/install gate identifies and removes mandatory product dependencies and core-to-product imports without breaking provider entry points | Planned | W1c |
| W2 | Public extension reference | A copyable external extension uses only public SDK/API from manifest through installed CLI execution; its documentation command runs in an isolated environment | Planned | W1 |
