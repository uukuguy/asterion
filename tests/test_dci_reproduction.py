...
from asterion.capabilities.dci.implementation.reproduction.paper_benchmarks import canonical_sha256
...
class TestDciRunManifestCompiler(unittest.TestCase):
    ...
    def test_write_run_manifest_close_failure_attempts_cleanup_and_allows_retry(
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                patch("asterion.capabilities.dci.implementation.reproduction.reproduction.os.open", side_effect=open_spy),
                patch("asterion.capabilities.dci.implementation.reproduction.reproduction.os.close", side_effect=close_spy),
    ...
    def test_write_run_manifest_cleanup_faults_do_not_mask_primary_failure(
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                patch("asterion.capabilities.dci.implementation.reproduction.reproduction.os.open", side_effect=open_spy),
                patch("asterion.capabilities.dci.implementation.reproduction.reproduction.os.fsync", side_effect=fsync_spy),
                patch("asterion.capabilities.dci.implementation.reproduction.reproduction.os.close", side_effect=close_spy),
                patch("asterion.capabilities.dci.implementation.reproduction.reproduction.os.unlink", side_effect=unlink_spy),
                self.assertRaisesRegex(
                    ValueError, "^DCI reproduction manifest write failed$"
                ) as raised,
            ):
