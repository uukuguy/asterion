"""Real P7 framed bridge against the pinned Prime SDK checkout."""
...
from asterion.applications.prime_agent.operator import (
    p7_development_sdk_provider as provider_contract,
)
from asterion.applications.prime_agent.operator.p7_development_gateway import (
    PrimeP7DevelopmentGateway,
)


_ROOT = Path(__file__).resolve().parents[1]
_PRIME = _ROOT / "3th-party" / "prime-agent"
_MAIN = _ROOT / "packages/typescript/prime-gateway/dist/src/p7-development-main.js"


class TestPrimeP7BuiltMainGateway(unittest.TestCase):
    def test_three_prompts_use_one_real_session_with_six_by_three_witness(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="asterion-p7-built-main-") as temporary:
            workspace = Path(temporary)
            gateway = PrimeP7DevelopmentGateway(
                node_bin="node", entrypoint=_MAIN, deadline_seconds=15,
            )
            gateway.bind(model_hook=model_hook, tool_hook=tool_hook)
            gateway.open_sync(
                run_id="run-p7-built-main",
                session_id="session-p7-built-main",
                generation=1,
