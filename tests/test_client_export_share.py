...
def _authority() -> ClientExportAuthority:
    return ClientExportAuthority(
        authority_id="export-authority-1", client_id="client-1", session_id="session-1",
...
class TestClientExportShare(unittest.TestCase):
    ...
    def test_private_export_authority_is_exact_and_one_use(self) -> None:
        ...
        with self.assertRaises(ClientExportError):
            export_client_session(_events(), visibility="private", artifacts=store, authority=authority, private_values=_private_values(backend))
    ...
    def test_share_is_receipted_once_and_no_service_returns_the_local_opaque_reference(self) -> None:
        ...
        shares = _Shares()
        shared = share_client_export(artifact, authority=authority, shares=shares)
        self.assertEqual(shared.share_ref, "share-ref-1")
        with self.assertRaises(ClientExportError):
            share_client_export(artifact, authority=authority, shares=shares)
