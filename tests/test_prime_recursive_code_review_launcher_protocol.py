...
def _frames() -> tuple[bytes, ...]:
    ...
    return (
        ...
        _frame(8, "follow-up-result", {"child_role_id": RECURSIVE_CODE_REVIEW_P3_FOLLOW_UP_ROLE_ID, "follow_up_digest": _DIGESTS[8]}),
...
class TestRecursiveCodeReviewLauncherProtocol(unittest.TestCase):
    def test_parses_only_the_full_causal_sequence(self) -> None:
        trace = parse_recursive_code_review_frames(b"\n".join(_frames()))
    ...
    def test_rejects_wrong_order_identity_cap_deadline_and_second_terminal(self) -> None:
        cases = (
            b"\n".join(_frames()[3:]),
            b"\n".join(_frames()[:3] + (_frame(3, "child-admitted", {"child_role_id": "prime.ipython-coding", "child_role_sha256": _DIGESTS[2], "child_usage_sha256": _DIGESTS[3]}),)),
            b"x" * (RECURSIVE_CODE_REVIEW_MAX_FRAME_BYTES + 1),
            b"\n".join(_frames() + (_frames()[-1],)),
        )
        for data in cases:
        ...
        with self.assertRaises(RecursiveCodeReviewReleaseError):
            parse_recursive_code_review_frames(b"\n".join(_frames()), clock=iter((0.0, 2.0)).__next__)
