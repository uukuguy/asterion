...
_CHILD = _P5_CHILD.replace("p5-development", "p6-development")


class TestPrimeP6DevelopmentGateway(unittest.TestCase):
    def test_three_prompts_have_one_terminal_six_by_three_witness(self) -> None:
        from asterion.applications.prime_agent.operator.p6_development_gateway import PrimeP6DevelopmentGateway

        with tempfile.TemporaryDirectory() as temporary:
            entrypoint = Path(temporary) / "bridge.js"
            entrypoint.write_text(_CHILD, encoding="utf-8")
            gateway = PrimeP6DevelopmentGateway(node_bin="node", entrypoint=entrypoint)
