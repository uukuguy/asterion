...
class TestP6DevelopmentSdkProvider(unittest.TestCase):
    def test_six_turn_deterministic_tool_policy(self) -> None:
        from asterion.applications.prime_agent.operator.p6_development_sdk_provider import P6_PROVIDER_OUTPUT_LIMITS, _deepseek_payload

        request = {"context": {"systemPrompt": "s", "messages": [], "tools": [{"name": "ipython", "description": "d", "parameters": {"type": "object"}}]}}
        for turn, ceiling in enumerate(P6_PROVIDER_OUTPUT_LIMITS):
