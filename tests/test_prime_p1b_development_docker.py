...
from asterion.applications.prime_agent.operator.p1b_development_docker import (
    ...
    P1BDockerCliTransport,
    _reap_process,
)
from asterion.applications.prime_agent.operator import (
    p1b_development_docker as p1b_docker,
)
...
from asterion.applications.prime_agent.operator import (
    p1_development_snapshot as snapshots,
)
...
class _Channel:
    ...
    async def finish(self, **_: object) -> P1BDockerCompletion:
        return P1BDockerCompletion(
            PRIME_IPYTHON_CODING_P1B_DEVELOPMENT_WORKLOAD_DIGEST, 1, 2, 12
        )
...
class _Transport:
    ...
    async def channel(self, *_: object, **__: object) -> _Channel:
        self.calls.append("attach")
        return self.channel_value

    async def snapshot(self, *_: object, **__: object) -> bytes:
        self.calls.append("snapshot")
        return b"p1b continuity fixture\n"

    async def initial_snapshot(self, *_: object, **__: object) -> bytes:
        self.calls.append("initial_snapshot")
        return b"def answer() -> int:\n    return 0\n"
    ...
    async def assert_absent(self, *_: object, **__: object) -> None:
        ...
        [
            {
                ...
                "CapAdd": None,
                "CapDrop": ["ALL"],
                "SecurityOpt": ["no-new-privileges:true", "seccomp=profile"],
                "Binds": None,
                "VolumesFrom": None,
                "Tmpfs": {
...
class TestP1BDockerPersistentWorkerService(unittest.IsolatedAsyncioTestCase):
    async def test_p1b_inspect_accepts_reordered_environment_and_rejects_invalid_keys(
        self,
    ) -> None:
        container_id = "d" * 64
        ...
        valid = list(
            reversed(
                [
                    *docker_cli._ENVIRONMENT,
                    *docker_cli._CLEARED_BASE_IMAGE_ENVIRONMENT,
                ]
            )
        )
        cases = (
    ...
    def test_local_root_snapshot_transport_requires_explicit_same_guest_confirmation(
        self,
    ) -> None:
            ...
            for platform, euid, confirmed in (
                ...
                with (
                    ...
                    with self.assertRaisesRegex(
                        Exception, "restricted worker value is invalid"
                    ):
                        P1BDevelopmentSnapshotTransport(
    ...
    async def test_proc_snapshots_use_fixed_paths_and_validate_continuity_before_solution(
        self,
    ) -> None:
        container = "d" * 64
        ...
        async def call(
            argv: tuple[str, ...], *_: object, **__: object
        ) -> DockerCliResult:
            calls.append(argv)
            if "inspect" in argv:
                return DockerCliResult(
        ...
        with (
            ...
            patch.object(
                snapshots.os, "open", side_effect=[11, 12, 13, 14, 15, 16, 17, 18]
            ) as opened,
            patch.object(
                snapshots.os,
                "pread",
                side_effect=lambda fd, _count, _offset: contents[fd],
            ),
            patch.object(
                snapshots,
                "_stable_regular_file",
                side_effect=lambda fd: (1, fd, len(contents[fd]), 0, 0),
            ),
            patch.object(snapshots.os, "close"),
        ):
            initial = await subject.initial_snapshot(
                container, control=_LifecycleCallControl(monotonic() + 30, None)
            )
            final = await subject.snapshot(
                container, control=_LifecycleCallControl(monotonic() + 30, None)
            )
    ...
    async def test_inspect_rejection_after_create_still_removes_and_proves_absence(
        self,
    ) -> None:
        transport = _Transport()
        transport.inspect_error = True
        service = P1BDockerPersistentWorkerService(
            image_digest="sha256:" + "a" * 64,
            transport=transport,
            run_id="run",
    ...
    async def test_cleanup_failure_retains_owner_for_retry(self) -> None:
        ...
        await service.acquire()
        transport.remove_error = True
        with self.assertRaises(ValueError):
            await service.cleanup()
        transport.remove_error = False
        await service.cleanup()
        self.assertEqual(transport.calls[-3:], ["remove", "remove", "absent"])

    async def test_acquire_cancellation_completes_cleanup_then_propagates_cancellation(
        self,
    ) -> None:
        transport = _Transport()
        transport.inspect_started = asyncio.Event()
        transport.allow_inspect = asyncio.Event()
        transport.remove_started = asyncio.Event()
        transport.allow_remove = asyncio.Event()
        service = P1BDockerPersistentWorkerService(
            image_digest="sha256:" + "a" * 64,
            transport=transport,
            run_id="run",
    ...
    async def test_uncertain_create_compensation_removes_provisional_name_then_asserts_absence(
        self,
    ) -> None:
        class Runner:
            def __init__(self) -> None:
            ...
            async def run(
                self, *, argv: tuple[str, ...], **_: object
            ) -> DockerCliResult:
                self.argv.append(argv)
                if "rm" in argv:
                    return DockerCliResult(0, b"prime-p1b-provisional\n", b"")
                return DockerCliResult(
                    1, b"", b"Error: No such object: prime-p1b-provisional\n"
                )
        ...
        self.assertEqual(
            runner.argv[0][-4:], ("container", "rm", "--force", "prime-p1b-provisional")
        )
        self.assertEqual(
            runner.argv[1][-5:],
            ("container", "inspect", "--format", "{{.Id}}", "prime-p1b-provisional"),
        )

    async def test_uncertain_create_compensation_waits_for_late_create_then_removes_it(
        self,
    ) -> None:
        class Clock:
            now = 0.0
        ...
        with (
            ...
            patch.object(p1b_docker, "monotonic", clock),
            patch.object(
                docker_cli,
                "monotonic",
                clock,
